  ...
  return (
    <>
      ...
      >
        ...
        <div className="absolute left-full top-1/2 -translate-y-1/2 ml-4 opacity-0 group-hover:opacity-100 transition-opacity z-10">
          <button
            ...
            className={`w-8 h-8 rounded-full flex items-center justify-center transition-all shadow-lg border-2 ${
              isConfirmingDelete 
      ...
      >
        ...
        <button
          onClick={handleDeleteClick}
          className={`absolute top-4 right-4 w-7 h-7 rounded-full flex items-center justify-center transition-all shadow-sm border ${
            isConfirmingDelete 
...
const ProjectList: React.FC<ProjectListProps> = ({ projects, user, onSelectProject, onNewProject, onManageTeam, onDeleteProject, isLoading }) => {
    ...
    <div className="max-w-[1800px] mx-auto px-4 md:px-6 py-6 md:py-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 md:mb-8">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto overflow-x-auto no-scrollbar pb-1">
          ...
          >
            <i className="fa-solid fa-users text-blue-600"></i><span className="whitespace-nowrap">팀 멤버 관리</span>
          </button>
          <button 
            onClick={exportToCSV}
            className="shrink-0 bg-white border-2 border-slate-100 text-slate-700 px-4 py-2 md:px-5 md:py-2.5 rounded-xl text-xs md:text-sm font-bold hover:border-black transition-all flex items-center gap-2"
